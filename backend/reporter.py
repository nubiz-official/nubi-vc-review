...
- This is internal review draft; regulatory/recurring revenue/comparative analysis requires human validation"""
    ...
    def _generate_5stage_scorecard(self, company_name: str, phase1: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Get evidence from each stage
        stage_1_evidence = base_scores.get("stage_1_원천기술통제", {}).get("evidence", [])
        stage_2_evidence = base_scores.get("stage_2_규제통제", {}).get("evidence", [])
        stage_3_evidence = base_scores.get("stage_3_플랫폼확장", {}).get("evidence", [])
        stage_4_evidence = base_scores.get("stage_4_반복매출", {}).get("evidence", [])
        stage_5_evidence = base_scores.get("stage_5_RWW개입", {}).get("evidence", [])
